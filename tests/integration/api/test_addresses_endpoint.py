...
class TestCreateAddressesEndpoint:
    ...
    async def test_create_addresses_success(self, test_client: TestClient):
        """
        ...
        # Act
        response = test_client.post("/api/v1/addresses/", json={"count": 2})

        # Assert
        assert response.status_code == 201  # TODO: Avoid magic numbers
    ...
    async def test_create_addresses_fails_with_invalid_count(self, test_client: TestClient, invalid_count: int):
        """
        Scenario 2: Tests that the request fails with an invalid count.
        """
        # Act:
        response = test_client.post(
            "/api/v1/addresses/", json={"count": invalid_count})

        # Assert
        # Pydantic validation should return a 422 error
        assert response.status_code == 422

    async def test_create_addresses_handles_service_error(self, test_client: TestClient):
        """
        ...
        # Act
        response = test_client.post("/api/v1/addresses/", json={"count": 5})
...
class TestListAddressesEndpoint:
    ...
    async def test_list_addresses_success(self, test_client: TestClient):
        """
        ...
        # Act
        response = test_client.get("/api/v1/addresses/")
    ...
    async def test_list_addresses_returns_empty_list(self, test_client: TestClient):
        """
